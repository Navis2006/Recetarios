...
interface RegisterPageProps {
  onRegister: (nombre: string, contrasena: string) => void;
  onNavigateToLogin: () => void;
}

const RegisterPage: FC<RegisterPageProps> = ({ onRegister, onNavigateToLogin }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-200px)]">
      <div className="bg-white p-8 md:p-12 rounded-xl shadow-2xl max-w-md w-full mx-4 animate-fade-in">
        <h1 className="text-3xl font-bold text-[#2C3E50] mb-6 text-center">Crear Cuenta</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-lg font-bold text-[#333333] mb-2">Confirmar Contraseña</label>
            <input 
              type="password" 
        ...
        <div className="text-center mt-6">
            <p className="text-[#555555]">
                ...
                <button onClick={onNavigateToLogin} className="font-bold text-[#FF6B6B] hover:underline">
                    Inicia sesión aquí
                </button>
            </p>
        </div>
